...
def change_theme():
    ...
    if _variable.get() == "Light Theme":
        ...
        for radiobutton in radio_list:
            radiobutton.configure(
                bg=bg_color, fg=fg_color, activebackground=bg_color, selectcolor="cyan"
            )
    ...
    # For Dark Theme
        ...
        for radiobutton in radio_list:
            radiobutton.configure(
                bg=bg_color, fg=fg_color, activebackground=bg_color, selectcolor="red"
            )
...
# Frames
# menu_frame = Frame(root, borderwidth=0, bg=bg_color)    # First frame in Root window - Menu_frame
main_frame = Frame(
    root, borderwidth=0, bg=bg_color
)  # Second frame in Root window - Main_frame
screen_frame = Frame(
    root, borderwidth=0, bg=bg_color
)  # Third frame in Root window - Screen_frame
Mid_frame = Frame(
    root, borderwidth=0, bg=bg_color
)  # Forth frame in Root window - Mid_frame
...
# Menu Toggle switch
# Top_button = Button(menu_frame, height=1, width=1,  # font=sci_font, command=sci_cal
#                     cursor="hand2", activebackground=bg_color, activeforeground=fg_color,
#                     text='=', bd=0, bg=bg_color, fg=fg_color )
...
def calculate_sc_inv(event):
    ...
    if btn_text == "sin⁻¹":
        ...
        try:
            ...
            screen.delete(0, END)
            screen.insert(0, "Value Overflow")
            return
    elif btn_text == "eˣ":
        try:
